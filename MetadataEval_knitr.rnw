...
#testarea <- readOGR(testareapath, testAreaName) # removing this as its probably not needed anymore.
...
\begin{itemize}
    ...
    \setlength{\parsep}{0pt}
<<DataSourcesList, results="asis", echo = FALSE>>=
sdm.dataSources$ProgramName <- sort(sdm.dataSources$ProgramName)
for(i in 1:length(sdm.dataSources$ProgramName)){
  x <- paste("\\item ", sdm.dataSources$ProgramName[[i]], "\n", sep = "")
  y <- sub("&", "\\\\&", x) #escape ampersands if there are any - special character in latex
  cat(y)
...
=======
>>>>>>> 1343e1509d4a59fe2e12c9368ae2c5512c87af9e
