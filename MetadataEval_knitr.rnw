...
<<pPlotFig, fig.width=6.5, fig.height=4, include=FALSE, echo=FALSE>>=
par(tcl=-0.2,   #tic length
    cex=0.6,     #text size # doesn't seem to apply!
    mgp=c(1.6,0.4,0) #placement of axis title, labels, line
    )
...
#change to three rows of three
# layout(matrix(c(19,2,4,6,19,1,3,5,19,8,10,12,19,7,9,11,19,14,16,18,19,13,15,17), 
# 		nrow = 6, ncol = 4, byrow = TRUE), 
# 		widths = c(0.15,1,1,1),heights=c(1,3,1,3,1,3))
layout(matrix(c(19,2,4,6,20,19,1,3,5,20,19,8,10,12,20,19,7,9,11,20,19,14,16,18,20,19,13,15,17,20), 
		nrow = 6, ncol = 5, byrow = TRUE), 
		widths = c(0.15,1,1,1,0.1),heights=c(1,3,1,3,1,3))
...
for (plotpi in 1:8){
  ...
  if(is.character(pPlots[[plotpi]]$x)){
    # legacy for factored env vars. There are none in Regional project
		barplot(pPlots[[plotpi]]$y, width=rep(1, length(pPlots[[plotpi]]$y)), col="grey",
							  ...
							  cex.names=0.7, las=2)	
	  plot(1,1,axes=FALSE, type="n", xlab=NA, ylab=NA)
	} else {
	  # get min, max of absence data
    abs.min <- min(abs.dat[,pPlots[[plotpi]]$gridName])
  	abs.max <- max(abs.dat[,pPlots[[plotpi]]$gridName])
	  # get min, max of presence data
  	p.min <- min(pres.dat[,pPlots[[plotpi]]$gridName])
    p.max <- max(pres.dat[,pPlots[[plotpi]]$gridName])
    #xrange <- c(p.min,p.max)
    # chop the partial plot data
    pplot.x <- pPlots[[plotpi]]$x[pPlots[[plotpi]]$x >= p.min & pPlots[[plotpi]]$x <= p.max]
    pplot.y <- pPlots[[plotpi]]$y[pPlots[[plotpi]]$x >= p.min & pPlots[[plotpi]]$x <= p.max]
    
    needTopGap <- FALSE
    # do we need a gap at top?
    if(abs.max > p.max){
      needTopGap <- TRUE
      # get 1/30 full range of abs
      minTopEnd <- abs.max - (abs.max-abs.min)/30
      if(minTopEnd < p.max){
        minTopEnd <- abs.max - (abs.max-p.max)*0.2
      }
      topGap <- c(p.max, minTopEnd)
    }
    needBottomGap <- FALSE
    if(p.min > abs.min){
      needBottomGap <- TRUE
      maxBottomEnd <- abs.min + (abs.max-abs.min)/30
      if(maxBottomEnd > p.min){
        maxBottomEnd <- abs.min + (p.min - abs.min)*0.2
      }
      bottomGap <- c(maxBottomEnd, p.min)
    }

    if(needBottomGap & needTopGap){
      gaps <- c(bottomGap, topGap)
    } else if(needBottomGap){
      gaps <- bottomGap
    } else if(needTopGap){
      gaps <- topGap
    } else { gaps <- c(abs.max, abs.max)}
    
    ## calculate gap at bottom
    

  	# tgh.gap.plot(pplot.x, pplot.y,
  	#  gap.axis = "x", gap = gaps,
  	# 	type = "l", xlim = c(abs.min, abs.max))  
    xticspacing <- signif((p.max-p.min)/4,1)
    xticlocs <- c(abs.min, seq(signif(p.min,1), p.max, xticspacing), abs.max)
  	tgh.gap.plot(pplot.x, pplot.y,
  	 gap.axis = "x", gap = gaps,
  		type = "l", xlim = c(abs.min, abs.max),
  	 xtics = xticlocs,
    ...
    # chop the density data
    #p.dens.x <- pres.dens$x[pres.dens$x >= p.min & pres.dens$x <= p.max]
    #p.dens.y <- pres.dens$y[pres.dens$x >= p.min & pres.dens$x <= p.max]
    p.dens.x <- pres.dens$x
    p.dens.y <- pres.dens$y
  	
  	
  	par(mar=c(0,2,0.5,0.5))
  	  	#tgh.gap.plot(p.dens.x, p.dens.y,
  	gap.plot(p.dens.x, p.dens.y,
  	        gap.axis = "x", gap = gaps,
  					main=NA,xlab=NA,ylab=NA,xtics = c(min(p.dens.x),max(p.dens.x)),
  					ytics = c(min(p.dens.y), max(p.dens.y)),xticlab = NA,yticlab = NA,
  					col="blue", lwd=2, type = "l", xlim = c(abs.min, abs.max), 
  					ylim=c(0,max(c(abs.dens$y,pres.dens$y)))
  					)

  	gap.plot(abs.dens$x, abs.dens$y,
  	         gap.axis = "x", gap = gaps,
  								main=NA,xlab=NA,ylab=NA,xtics = NA,ytics = NA, xticlab = NA,yticlab = NA,
  					col="red", lwd=1, type = "l", xlim = c(abs.min, abs.max),
  					add = TRUE, ylim=c(0,max(c(abs.dens$y,pres.dens$y))))
  	#mtext(abs.min, side = 3, line = -1, adj = 1, cex = 0.6)
  	#mtext(abs.max, side = 3, line = -2, adj = 1, cex = 0.6)
	}
}
...
effects of the other variables removed \cite{liaw2002}. Higher values in the black line indicate where this variable is associated with 
appropriate habitat. The density distribution of each category (thin red = BG points, thick blue = PR points) is depicted at the top margin, within the range (min and max) of the PR points. The entire range of BG points may be larger; this range is depicted in the upper right corner of each figure.
