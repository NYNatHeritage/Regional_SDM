...
for (plotpi in 1:8){
  ...
  if(is.character(pPlots[[plotpi]]$x)){
	  ...
	  # plot only range of presence points
    abs.min <- min(abs.dat[,pPlots[[plotpi]]$gridName])
  	abs.max <- max(abs.dat[,pPlots[[plotpi]]$gridName])
	  xmini <- min(pres.dat[,pPlots[[plotpi]]$gridName])
    xmaxi <- max(pres.dat[,pPlots[[plotpi]]$gridName])
    # to get full range (range of absence pts) uncomment these
    #xmini <- min(abs.dat[,pPlots[[plotpi]]$gridName])
    #xmaxi <- max(abs.dat[,pPlots[[plotpi]]$gridName])
    xrange <- c(xmini,xmaxi)
  	# could use quantiles to get rid of outliers and show primary part of x range
  	#xrange <- quantile(pres.dat[,pPlots[[plotpi]]$gridName], probs = c(0.001, 0.999))
    #print(paste(pPlots[[plotpi]]$gridName, xmini, xmaxi, xrange[1],xrange[2],sep=", "))
    ##### caculate gap: from xmax to 1/10 of ymax
    # get 1/10 full range of abs
    minSecondSection <- abs.max - (abs.max-abs.min)/30
    
  	gap.plot(pPlots[[plotpi]]$x, pPlots[[plotpi]]$y,
  	 gap.axis = "x", gap = c(xmaxi, minSecondSection),
  	...
  	# xlim = xrange, ylim=c(0,max(c(abs.dens$y,pres.dens$y))),
